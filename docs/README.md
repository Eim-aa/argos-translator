...
An illustration of the workflow in a TextEdit-style window: a line of English is
selected, the Option key is double-tapped, and the Hammerspoon canvas pops up
with the Chinese translation plus the latency. The committed `demo.gif` is
generated by `render_demo.py` (below); a real screen recording is an alternative.
